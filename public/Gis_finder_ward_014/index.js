...
var surveyor_submitted_14_gisId = new Set();

function isGisIdSubmitted(gisId) {
    return surveyor_submitted_14_gisId.has(gisId);
}

window.updateSubmittedGisId = function(gisId) {
    surveyor_submitted_14_gisId.add(gisId);
    lyr_building_ppt.changed();
};

window.addEventListener('load', function() {
    try {
        const storedGisIds = JSON.parse(localStorage.getItem('surveyor_submitted_14_gisId') || '[]');
...
var popupEventTriggered = function(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        ...
        if (feature) {
            ...
            if (gisId) {
                localStorage.setItem('selectedGISID', "014-"+gisId);
